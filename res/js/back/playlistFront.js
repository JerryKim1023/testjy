...
function highlight(i, which, persist) {
  ...
  let PlaylistButtons = function() {
    ...
    this.autoplay = function(element) {
        ...
        console.log("Autoplay button clicked. Index: " + index);
        playlistAutoplay = true;
        autoplayList = getAutoplayList(index); // Ensure autoplayList is set
        console.log("Autoplay List: ", autoplayList); // Debugging
...
let playlistButtons = new PlaylistButtons;
  ...
  // Ensure getAutoplayList function retrieves the correct list
  function getAutoplayList(index) {
    // Implement your logic to retrieve the correct autoplay list based on the index
    console.log("Getting autoplay list for index: " + index);
    // Example logic to return a mock list
    return ["video1", "video2", "video3"]; // Replace with the actual list
}
